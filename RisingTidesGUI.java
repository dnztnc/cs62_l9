...
public class RisingTidesGUI {
    ...
    private File[] terrainFilesIn(File directory) {
        File[] results = directory.listFiles((File dir, String name) -> name.endsWith(".terrain"));
    ...
    private JComboBox<File> makeFileSelector() {
        JComboBox<File> result = new JComboBox<File>();
        for (File file: terrainFilesIn(TERRAIN_DIRECTORY)) {
            result.addItem(file);
        }
        return result;
    }
    ...
    private JButton makeLoadButton() {
        JButton result = new JButton("Load");
        ...
        new Thread() {
            public void run() {
                try {
                    ...
                    boolean[][] flooded = RisingTides.floodedRegionsIn(terrain.heights, terrain.sources, waterHeight);
